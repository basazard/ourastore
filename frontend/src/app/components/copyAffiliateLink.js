"use client";
import { usePathname } from "next/navigation";
import { toast } from "react-toastify";

export default function CopyAffiliateLink() {
  const pathname = usePathname();
  const copyUrl = () => {
    navigator.clipboard.writeText(
      `${process.env.NEXT_PUBLIC_FE_API_ROOT}${pathname}`
    );
    toast.success("Link afiliasi berhasil disalin", {
      autoClose: 1000,
      hideProgressBar: true,
    });
  };
  return (
    <div>
      <button
        onClick={copyUrl}
        className="bg-secondary w-full rounded-md text-secondary-foreground p-2"
      >
        <div className="flex flex-row gap-2 items-center justify-center">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            height="16px"
            viewBox="0 0 24 24"
            width="16px"
            fill="#FFFFFF"
          >
            <path d="M0 0h24v24H0V0z" fill="none" />
            <path d="M19 2h-4.18C14.4.84 13.3 0 12 0S9.6.84 9.18 2H5c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm7 18H5V4h2v3h10V4h2v16z" />
          </svg>
          <span className="text-xs font-extralight">Copy Affiliate Link</span>
        </div>
      </button>
    </div>
  );
}
